...
const formatMileage = (value: string) =>
  value ? new Intl.NumberFormat("pt-BR").format(Number(digitsOnly(value))) : "";
...
const deliveryOptions: Array<{ id: DeliveryOption; label: string }> = [
  ...
  { id: "2", label: "2 dias úteis" },
  { id: "3", label: "3 dias úteis" },
  { id: "5", label: "5 dias úteis" },
  ...
  return (
    <label className="field catalog-field">
      ...
      {open && !disabled && options.length > 0 && (
        <div className="catalog-options" role="listbox">
  ...
  return (
    <div className="customer-picker">
      <label className="field">
        <span>Buscar cliente por nome, telefone ou CPF</span>
        <div className={`combobox-input ${selectedId ? "has-value" : ""}`}>
          <Icon name="search" />
          <input
            value={query}
            placeholder="Digite para localizar um cliente"
            autoComplete="off"
            onFocus={() => setOpen(true)}
            onBlur={() => window.setTimeout(() => setOpen(false), 120)}
            onChange={(event) => {
      ...
      {open && (
        <div className="combobox-menu" role="listbox">
          {matches.slice(0, 7).map((customer) => (
            <button
              type="button"
              key={customer.id}
              onMouseDown={(event) => event.preventDefault()}
...
export function NewEntryModal({
  ...
  data: KaizoData;
  onClose: () => void;
  onSave: (entry: ServiceEntry, preparedData: KaizoData) => void;
}) {
  ...
  const [vehicleDraft, setVehicleDraft] = useState({
    ...
    year: "",
    color: "Prata",
    vehicleCategory: "other" as VehicleCategory,
    mileage: "",
    fuel: "",
  });
  ...
  const customerVehicles = workingData.vehicles.filter(
    (vehicle) => vehicle.customerId === customerId,
  );
  const selectedVehicle = workingData.vehicles.find(
    (vehicle) => vehicle.id === vehicleId,
  );
  const brandOptions = localVehicleCatalog.searchBrands(vehicleDraft.brand);
  const modelOptions = localVehicleCatalog.searchModels(
    vehicleDraft.brand,
    vehicleDraft.model,
  );
  const versionOptions = localVehicleCatalog.searchVersions(
    vehicleDraft.brand,
    vehicleDraft.model,
    vehicleDraft.version,
  );
  ...
  const saveCustomer = () => {
    if (!customerDraft.name.trim() || !customerDraft.phone.trim())
      return setError("Informe nome e telefone para cadastrar o cliente.");
    const customer: Customer = {
      id: createId("customer"),
      name: customerDraft.name.trim(),
      phone: customerDraft.phone.trim(),
      cpf: customerDraft.cpf.trim() || undefined,
      notes: customerDraft.notes.trim() || undefined,
      createdAt: new Date().toISOString(),
    };
    setWorkingData((current) => customerService.upsert(current, customer));
  ...
  const saveVehicle = () => {
    ...
    const vehicle: Vehicle = {
      id: createId("vehicle"),
      customerId,
      plate: vehicleDraft.plate.replace(/[^a-zA-Z0-9]/g, "").toUpperCase(),
      brand: vehicleDraft.brand.trim(),
      model: vehicleDraft.model.trim(),
      version: vehicleDraft.version.trim(),
      year,
      color: vehicleDraft.color.trim(),
      vehicleCategory: vehicleDraft.vehicleCategory,
      mileage: vehicleMileage,
      fuel: vehicleDraft.fuel || undefined,
    };
    setWorkingData((current) => vehicleService.upsert(current, vehicle));
  ...
  const chooseDelivery = (option: DeliveryOption) => {
    ...
    if (option === "today") setDeliveryDate(addDays(0));
      ...
      setDeliveryDate(addBusinessDays(Number(option)));
  };
  const submit = () => {
    if (!customerId) return setError("Selecione ou cadastre um cliente.");
    if (!vehicleId)
      return setError("Selecione ou cadastre um veículo deste cliente.");
    if (!mileage || Number(mileage) < 0)
      return setError("Informe uma quilometragem de entrada válida.");
    if (!problem.trim())
      return setError("Descreva o problema relatado ou o serviço solicitado.");
    if (!deliveryDate) return setError("Escolha uma previsão de entrega.");
    const now = new Date().toISOString();
    const number =
      Math.max(...workingData.entries.map((entry) => entry.number), 2000) + 1;
    onSave(
      {
        id: createId("entry"),
        number,
        ...
        timeline: [
          {
            id: createId("event"),
            date: now,
            action: "Atendimento criado",
            description: `Veículo ${selectedVehicle?.plate} recebido e relato inicial registrado.`,
          },
        ],
      },
      workingData,
  ...
  return (
    ...
    >
      <section
        className="modal modal-wide new-order-modal"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="modal-head">
          <div>
            ...
            <p>Cliente, veículo e entrada em um único fluxo.</p>
          </div>
          <button
        ...
        <div className="new-order-flow">
          <section className="flow-step">
            <div className="step-label">
              <b>1</b>
              <span>Cliente</span>
              {customerId && <em>Concluído</em>}
            </div>
            <CustomerPicker
              customers={workingData.customers}
            ...
            />
            {showCustomerForm && (
              <div className="inline-register">
                <div className="inline-register-head">
                  <div>
                    <strong>Novo cliente</strong>
                    <small>Cadastre sem sair do atendimento.</small>
                  </div>
                  <button
                    type="button"
                ...
                <div className="inline-register-actions">
                  <button
                    type="button"
          ...
          {customerId && (
            <section className="flow-step">
              <div className="step-label">
                <b>2</b>
                <span>Veículo do cliente</span>
                {vehicleId && <em>Concluído</em>}
              </div>
              {customerVehicles.length > 0 && (
                <div className="vehicle-choice-list">
                  {customerVehicles.map((vehicle) => (
                    <button
                      type="button"
                      className={vehicleId === vehicle.id ? "selected" : ""}
                      key={vehicle.id}
                    ...
                    >
                      ...
                      <em>{vehicle.mileage.toLocaleString("pt-BR")} km</em>
                    </button>
                  ))}
                </div>
              )}
              ...
              {showVehicleForm && (
                <div className="inline-register vehicle-register">
                  <div className="inline-register-head">
                    <div>
                  ...
                  <div className="form-grid">
                    ...
                    <Field label="Quilometragem">
                      <MileageInput
                        ...
                        onChange={(value) =>
                          setVehicleDraft({ ...vehicleDraft, mileage: value })
                        }
                      />
                    </Field>
          ...
          {vehicleId && !showVehicleForm && (
            <section className="flow-step">
              <div className="step-label">
                <b>3</b>
                <span>Entrada e diagnóstico inicial</span>
              </div>
              <div className="form-grid">
                <Field label="Quilometragem de entrada">
                  <MileageInput
                    value={mileage}
                    onChange={setMileage}
                    required
                ...
                <Field label="Problema relatado / descrição" span>
                  <textarea
                    rows={4}
                    value={problem}
                    onChange={(event) => setProblem(event.target.value)}
                    placeholder="Descreva o que o cliente relatou e os sintomas observados..."
                    required
                  />
                </Field>
                ...
                <Field label="Observações" span>
                  <input
                    ...
                    placeholder="Ex.: cliente aguarda retorno por WhatsApp"
                  />
                </Field>
              </div>
            </section>
          ...
          {error && (
            <p className="form-error new-order-error" role="alert">
              {error}
            </p>
          )}
          <div className="form-footer sticky-order-footer">
            <button
              type="button"
              className="secondary-button"
              onClick={onClose}
