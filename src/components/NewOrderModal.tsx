...
import type { Customer, KaizoData, Priority, ServiceOrder, Vehicle } from "@/lib/types";
...
export function NewOrderModal({ data, onClose, onSave }: { data: KaizoData; onClose: () => void; onSave: (order: ServiceOrder, preparedData: KaizoData) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal modal-wide new-order-modal" role="dialog" aria-modal="true" aria-label="Nova ordem de serviço"><div className="modal-head"><div><h2>Nova ordem de serviço</h2><p>Cliente, veículo e entrada em um único fluxo.</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="Fechar">×</button></div><div className="new-order-flow">
    ...
    {error && <p className="form-error new-order-error" role="alert">{error}</p>}<div className="form-footer sticky-order-footer"><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button type="button" className="primary-button" disabled={!vehicleId} onClick={submit}>CRIAR ORDEM DE SERVIÇO</button></div>
  </div></section></div>;
}
