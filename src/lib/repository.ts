...
import { inferVehicleCategory } from "./vehiclePresentation";
...
const normalizeBudget = (budget: Budget): Budget => ({
  ...
  parts: (budget.parts ?? []).map((part) => ({
    ...part,
    responsibility: part.responsibility ?? "Oficina",
...
const normalizeData = (data: KaizoData): KaizoData => ({
  ...data,
  company: { ...data.company, accent: "#E5484D" },
...
type LegacyOrder = Record<string, unknown> & {
  ...
  parts: Budget["parts"];
  labor: Budget["labor"];
  evidences?: ServiceEntry["evidences"];
  quoteMessage: string;
  createdAt: string;
  updatedAt: string;
  timeline: ServiceEntry["timeline"];
};
...
const migrateLegacyData = (raw: Record<string, unknown>): KaizoData => {
  ...
  legacyOrders.forEach((legacy, index) => {
    ...
    entries.push({
      id: entryId,
      number: 2000 + legacyOrders.length - index,
      customerId: legacy.customerId,
      vehicleId: legacy.vehicleId,
      mileageIn: legacy.mileageIn,
      reportedProblem: legacy.reportedProblem,
      technicalNotes: legacy.technicalNotes ?? "",
      recommendations: legacy.recommendations ?? "",
      tags: legacy.tags ?? [],
      priority: legacy.priority ?? "Normal",
      initialDueDate: legacy.dueDate,
      notes: legacy.notes ?? "",
      status: entryStatus,
      evidences: (legacy.evidences ?? []).map((evidence) => ({
        ...evidence,
        orderId: entryId,
      })),
    ...
    const budgetStatus: BudgetStatus =
      legacy.status === "Recusado" || legacy.status === "Cancelado"
        ? "Recusado"
        : legacy.status === "Aguardando aprovação" ||
            legacy.status === "Orçamento enviado"
          ? "Aguardando aprovação"
          : legacy.status === "Aprovado" ||
              ["Em serviço", "Finalizado", "Entregue"].includes(legacy.status)
            ? "Aprovado"
            : "Rascunho";
    budgets.push(
      normalizeBudget({
        id: budgetId,
        number: 3000 + legacyOrders.length - index,
        entryId,
        status: budgetStatus,
        parts: legacy.parts ?? [],
        labor: legacy.labor ?? [],
        quoteMessage: legacy.quoteMessage ?? "",
  ...
  const data: KaizoData = {
    company: raw.company as KaizoData["company"],
    customers: (raw.customers as KaizoData["customers"]) ?? [],
    vehicles: (raw.vehicles as KaizoData["vehicles"]) ?? [],
    entries,
    budgets,
    orders,
    payments: ((raw.payments as KaizoData["payments"]) ?? []).flatMap(
      (payment) =>
        orderIdMap.has(payment.orderId)
          ? [{ ...payment, orderId: orderIdMap.get(payment.orderId)! }]
          : [],
    ),
    appointments: [],
    postSales: [],
  ...
  load() {
    ...
    try {
      ...
      return Array.isArray(raw.entries)
        ? normalizeData(raw as unknown as KaizoData)
        : migrateLegacyData(raw);
    } catch {
      return cloneSeed();
    }
  },
...
export const customerService = {
  ...
  upsert: (data: KaizoData, customer: KaizoData["customers"][number]) => ({
    ...data,
...
export const vehicleService = {
  ...
  upsert: (data: KaizoData, vehicle: KaizoData["vehicles"][number]) => ({
    ...data,
    vehicles: data.vehicles.some((item) => item.id === vehicle.id)
      ? data.vehicles.map((item) => (item.id === vehicle.id ? vehicle : item))
      : [vehicle, ...data.vehicles],
  }),
  remove: (data: KaizoData, id: string) => ({
    ...data,
    vehicles: data.vehicles.filter((item) => item.id !== id),
  }),
};

export const settingsService = {
  update: (data: KaizoData, company: KaizoData["company"]) => ({
    ...data,
    company: { ...company, accent: "#E5484D" },
  }),
};
