...
def make_background():
    canvas = Image.new("RGB", (WIDTH, HEIGHT), "#0C0C0E")
    glow = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
    glow_draw = ImageDraw.Draw(glow)
    glow_draw.ellipse((1320, -350, 2180, 510), fill=(229, 72, 77, 42))
    glow_draw.ellipse((-480, 680, 380, 1540), fill=(255, 99, 105, 18))
...
def compose(title, subtitle, screens, output):
    ...
    draw.rounded_rectangle((64, 48, 76, 128), radius=6, fill="#E5484D")
    draw.text((98, 43), title, font=font(BOLD, 46), fill="#F7F7F8")
    draw.text((100, 105), subtitle, font=font(REGULAR, 21), fill="#A1A1AA")
    draw.text((1758, 55), "16:9", font=font(BOLD, 20), fill="#FF6369")
    draw.text((1686, 87), "1920 × 1080", font=font(REGULAR, 16), fill="#71717A")
    ...
    for index, (file_name, label) in enumerate(screens):
        ...
        draw.text((x + (target_w - label_w) // 2, label_y), label, font=font(BOLD, 18), fill="#F7F7F8")

    draw.line((64, 1020, 1856, 1020), fill="#29292F", width=1)
    draw.text((64, 1035), "KAIZO · Gestão automotiva local-first", font=font(REGULAR, 15), fill="#71717A")
    draw.text((1610, 1035), "Mobile · 390 × 844", font=font(REGULAR, 15), fill="#71717A")
...
def compose_master():
    canvas = make_background()
    draw = ImageDraw.Draw(canvas)
    draw.rounded_rectangle((64, 40, 76, 120), radius=6, fill="#E5484D")
    draw.text((98, 35), "KAIZO · Todas as telas", font=font(BOLD, 44), fill="#F7F7F8")
    draw.text((100, 94), "Visão completa do aplicativo mobile", font=font(REGULAR, 20), fill="#A1A1AA")
    draw.text((1758, 48), "16:9", font=font(BOLD, 20), fill="#FF6369")
    draw.text((1686, 80), "1920 × 1080", font=font(REGULAR, 16), fill="#71717A")
